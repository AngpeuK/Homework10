  ...
  test.describe('Positive tests', () => {
    ...
    test('1.2/1.3/1.4 Tests for Any RiskScore loanPeriod return should be adequate, should return status code 200', async ({
      ...
      if (responseBody.riskLevel === 'Low Risk') {
        ...
        console.warn(
          `1.2/1.3/1.4 Unexpected risk level, so no period provided, that is ok: ${responseBody.riskLevel}`,
        )
      }
    })
    ...
    test('1.2 Low Risk Score loanPeriod return should be adequate, should return status code 200', async ({
      ...
      if (responseBody.riskLevel === 'Low Risk') {
        ...
        console.warn(`1.2 inadequate risk level: ${responseBody.riskLevel}`)
      } else if (responseBody.riskLevel === 'High Risk') {
        console.warn(`1.2 inadequate risk level: ${responseBody.riskLevel}`)
      } else {
        // Print if Undocumented Risk Level
    ...
    test('1.3 Medium Risk Score loanPeriod return should be adequate, should return status code 200', async ({
      request,
    }) => {
      const requestBody = ApplicationDto.createMediumRiskApplication()
      // console.log('requestBody for 1.3:', requestBody) //Print Request Body
      //Request
      const response = await request.post(RequestURL, {
        data: requestBody,
      })
      ...
      expect.soft(responseBody.riskDecision).toBeDefined()
      //Print Response Body and status code
      // console.log('ResponseBody for 1.3:', responseBody)
      // console.log('Response status code for 1.3:', response.status())

      if (responseBody.riskLevel === 'Medium Risk') {
        expect.soft(responseBody.riskPeriods).toEqual(expect.arrayContaining([6, 9, 12]))
      } else if (responseBody.riskLevel === 'Low Risk') {
        console.warn(`1.3 inadequate risk level: ${responseBody.riskLevel}`)
      } else if (responseBody.riskLevel === 'High Risk') {
        console.warn(`1.3 inadequate risk level: ${responseBody.riskLevel}`)
      } else {
        // Print if Undocumented Risk Level
    ...
    test('1.4 Medium Risk Score loanPeriod return should be adequate, should return status code 200', async ({
      request,
    }) => {
      const requestBody = ApplicationDto.createHighRiskApplication()
      // console.log('requestBody for 1.4:', requestBody) //Print Request Body
      //Request
      const response = await request.post(RequestURL, {
        data: requestBody,
      })
      ...
      expect.soft(responseBody.riskDecision).toBeDefined()
      //Print Response Body and status code
      // console.log('ResponseBody for 1.4:', responseBody)
      // console.log('Response status code for 1.4:', response.status())

      if (responseBody.riskLevel === 'High Risk') {
        expect.soft(responseBody.riskPeriods).toEqual(expect.arrayContaining([3, 6]))
      } else if (responseBody.riskLevel === 'Medium Risk') {
        console.warn(`1.4 inadequate risk level: ${responseBody.riskLevel}`)
      } else if (responseBody.riskLevel === 'Low Risk') {
        console.warn(`1.4 inadequate risk level: ${responseBody.riskLevel}`)
      } else {
        // Print if Undocumented Risk Level
